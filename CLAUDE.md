...
This repository is the **hackingtool** Claude Code plugin: 183+ pentesting and OSINT tools wired into Claude via a single skill.

## Plugin root

```
CLAUDE_PLUGIN_ROOT=/home/user/hackingtool-plugin/plugins/hackingtool
```

All wrapper scripts live at `$CLAUDE_PLUGIN_ROOT/scripts/`. All skill instructions are in `$CLAUDE_PLUGIN_ROOT/skills/pentest/SKILL.md`.
...
```bash
python /home/user/hackingtool-plugin/plugins/hackingtool/scripts/ht_preflight.py
```

Read the `verdict` and act:

- **`ready`** → state the backend in one sentence (e.g. "Running native on Linux" / "Running via Docker") and start work.
...
Anti-patterns: `for`-looping curl across many paths instead of `ffuf` / `nuclei`; hand-parsing TLS output instead of `nuclei -tags ssl`; saying "I don't have nuclei" when preflight returned `ready`.
...
All at `/home/user/hackingtool-plugin/plugins/hackingtool/scripts/`. Emit JSON.
...
2. **Read the ask** — map to a workflow and apply the defaults table.
...
- `/home/user/hackingtool-plugin/plugins/hackingtool/skills/pentest/reference/workflows.md` — named playbooks
- `/home/user/hackingtool-plugin/plugins/hackingtool/skills/pentest/reference/runtime-fallbacks.md` — templates per fallback reason
